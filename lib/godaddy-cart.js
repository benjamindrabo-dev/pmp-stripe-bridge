...
import {normalizeGoDaddyAttribution} from './godaddy-attribution.js';
import {snapshotToInput,resolveLocale,cartError} from '../public/godaddy-cart-contract.js';
import {CHECKOUT_COUNTRIES,purchasableStatus,checkoutCountry,snapshotForCountry} from '../public/godaddy-country-contract.js';
import {appliedDiscountCodes} from '../public/godaddy-cart-snapshot.js';
...
import {recalculateShopifyCart,repriceShopifyCart} from './godaddy-storefront-cart.js';
...
export async function createFromSnapshot(body){
 ...
 const input=snapshotToInput(actual,context);applyCatalog(input,await catalog(input.items.map(i=>i.variant_id),input.country));
 input.sourceSnapshot=actual;
 input.attribution={...normalizeGoDaddyAttribution(body.attribution),shopify_cart_token:supplied.shopifyCartToken};
 input.promotionCode=null;input.skipAutomaticPromotion=true;
...
export async function dentalSuggestion(cart){
 ...
 // Hidden direct-link products are purchasable, but are not recommendations.
...
export function expandedQuote(cart){
 ...
 return {...q,countries:CHECKOUT_COUNTRIES,canChangeCountry:!cart.supersededBy,locale:resolveLocale(cart.locale),source:cart.source||null,shopifyRecalculated:cart.shopifyRecalculated===true,sourceCartSubtotal:cart.sourceCartSubtotal??null,items:cart.items.map((i,index)=>({variantId:i.variant_id,lineKey:String(i.sourceLine??index)+':'+i.price_cents,title:i.title,quantity:i.quantity,unitMinor:i.original_price_cents,lineTotalMinor:i.price_cents*i.quantity,originalLineTotalMinor:i.original_price_cents*i.quantity,gift:i.price_cents===0,addon:i.addon===true,image:i.image||null})),supersededBy:cart.supersededBy||null,wallets:{applePay:false,googlePay:false,reason:'merchant-domain-and-payment-flow-not-verified'}};
}
async function changeCountry(cart,body){
 const country=checkoutCountry(body.country);
 if(country===cart.country)return expandedQuote(cart);
 const actual=await repriceShopifyCart(snapshotForCountry(cart),{country,locale:cart.locale});
 const input=snapshotToInput(actual,{country,locale:cart.locale});
 applyCatalog(input,await catalog(input.items.map(i=>i.variant_id),country));
 const addons=new Set(cart.items.filter(i=>i.addon).map(i=>i.variant_id));
 input.items=input.items.map(i=>({...i,addon:addons.has(i.variant_id)}));
 const included=appliedDiscountCodes(actual);
 const promotionCode=included.includes(cart.promotionCode)?null:cart.promotionCode;
 const result=await createGoDaddyQuote({...cart,...input,sourceSnapshot:actual,promotionCode,email:typeof body.email==='string'?body.email:cart.email,source:'shopify_country_change',sourceCartSubtotal:input.cartSubtotalMinor,sourceItemCount:input.sourceItemCount,shopifyRecalculated:true,skipAutomaticPromotion:true});
 const next=await loadCart(result.sessionId);
 cart.supersededBy=result.sessionId;await storage.set('cart:'+cart.id,cart);
 return expandedQuote(next);
}
export async function revise(id,body){
 if(!canPrepare())throw cartError('CHECKOUT_DISABLED');const lock=prefix()+'cart-edit:'+id,token=randomUUID();if(await redis(['SET',lock,token,'NX','EX','120'])!=='OK')throw cartError('CART_BUSY');
 ...
 if(body.action==='change-country')return await changeCountry(cart,body);
