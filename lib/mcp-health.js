...
async function testGoogleAds(accessToken, customerId, loginCustomerId = '') {
  ...
  if (!response.ok) {
    ...
    throw new Error(JSON.stringify(safeDetails));
  }
  const customer = data?.results?.[0]?.customer || {};
  return { ok: true, customer_id: customer.id || customerId };
}
...
export default async function handler(req, res) {
  ...
  const result = { ok: Object.values(configured).every(Boolean), service: 'Pure Majesty Google Ads + GA4 MCP', version: '1.3.5-detailed-ads-error', google_ads_api_version: 'v25', configured, non_secret_configuration: cfg };
