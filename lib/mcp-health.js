...
async function googleAdsConversionActions(token, customerId) {
  const query = `SELECT conversion_action.id, conversion_action.name, conversion_action.category, conversion_action.type, conversion_action.status, conversion_action.primary_for_goal, conversion_action.counting_type FROM conversion_action WHERE conversion_action.status != 'REMOVED' ORDER BY conversion_action.name`;
  ...
  return (data.results || []).map((row) => {
    ...
    return {
      ...
      primary_for_goal: Boolean(a.primaryForGoal),
      counting_type: a.countingType || null,
    };
  }).filter((a) => /purchase|cart|view|checkout|page/i.test(`${a.name || ''} ${a.category || ''}`));
}

async function ga4EcommerceEvents(token, propertyId) {
  const response = await fetch(`https://analyticsdata.googleapis.com/v1beta/properties/${propertyId}:runReport`, {
    ...
    body: JSON.stringify({
      ...
      metrics: [{ name: 'eventCount' }, { name: 'totalUsers' }, { name: 'purchaseRevenue' }],
      dimensionFilter: {
        filter: {
          fieldName: 'eventName',
...
export default async function handler(req, res) {
  ...
  if (String(req.query?.tracking || '') === '1') {
    result.tracking_audit = { google_ads_conversion_actions: null, ga4_events_30d: null };
    try {
      const token = await googleToken(cfg);
      try {
        result.tracking_audit.google_ads_conversion_actions = await googleAdsConversionActions(token, String(cfg.google_ads_customer_id).replace(/\D/g, ''));
      } catch (error) {
        result.tracking_audit.google_ads_conversion_actions = { error: error.message };
      }
      try {
        result.tracking_audit.ga4_events_30d = await ga4EcommerceEvents(token, String(cfg.ga4_property_id).replace(/\D/g, ''));
      } catch (error) {
        result.tracking_audit.ga4_events_30d = { error: error.message };
      }
    } catch (error) {
      result.tracking_audit.oauth_error = error.message;
    }
  }
