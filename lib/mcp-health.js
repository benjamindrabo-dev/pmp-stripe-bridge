...
async function googleAdsConversionActions(token, customerId) {
  ...
  const data = await response.json();
  if (!response.ok) throw new Error(JSON.stringify(data?.error || { status: response.status }));
...
export default async function handler(req, res) {
  ...
  const result = { ok: Object.values(configured).every(Boolean), service: 'Pure Majesty Google Ads + GA4 MCP', version: '1.4.2-tracking-audit', google_ads_api_version: 'v25', configured };
  if (String(req.query?.tracking || '') === '1') {
    ...
    try {
      ...
      try { result.tracking_audit.google_ads_conversion_actions = await googleAdsConversionActions(token, String(cfg.google_ads_customer_id).replace(/\D/g, '')); }
      catch (error) { result.tracking_audit.google_ads_conversion_actions = { error: error.message }; }
      try { result.tracking_audit.ga4_events_30d = await ga4EcommerceEvents(token, String(cfg.ga4_property_id).replace(/\D/g, '')); }
