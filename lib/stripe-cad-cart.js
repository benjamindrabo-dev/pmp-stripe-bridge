...
import {get,set,redis,release,error,shopify,moneyScale} from './square-bridge.js';
...
export async function reviseCart(id,body){
 ...
 try{
  ...
  const attempt=await get('stripe:attempt:'+id);if((attempt&&!attempt.failed)||await get('done:'+id))throw error('Payment is already being confirmed.',409);
  ...
  const next=await createStripeQuote({...cart,items,country,displayCurrency,scale,catalogPrices,email:body.email||cart.email,promotionCode:body.promotionCode??cart.promotionCode});
