...
import {loadStripePaymentCurrencies} from './stripe-payment-currencies.js';
import {ensureStripeWalletDomain} from './stripe-wallet-domain.js';
import countries from './square-countries.js';
import {createFxQuote} from './square-fx.js';
import {stripePaymentQuote,assertStripeQuote,oxxoVoucher} from './stripe-payment-quote.js';
...
export async function ensureStripeReady(){
 ...
 // Missing wallet setup must never disable otherwise healthy card payments.
 try{await ensureStripeWalletDomain(stripe);}catch{console.warn('Stripe wallet domain setup unavailable');}
...
export async function createStripeQuote(input) {
  ...
  // Preserve the active Stripe promotions and the first-order restriction.
  if(code && !['WELCOME20','WELCOME10','THANK10'].includes(code))throw error('This promotion code is not available.',400);
  if(code==='THANK10')await assertFirstOrder(input.email);
  const discount=code==='WELCOME20'?0.8:['WELCOME10','THANK10'].includes(code)?0.9:1;
  ...
  const accountingQuote=createFxQuote({displayCurrency:input.displayCurrency,displayAmount:major(total,input.scale),rates:fx});
  const supportedCurrencies=await loadStripePaymentCurrencies(stripe);
  const quote=stripePaymentQuote(accountingQuote,input.country,total,input.scale,supportedCurrencies);
  const id='st_'+crypto.randomUUID().replaceAll('-','');
  const cart={...input,locale:input.locale&&input.locale!=='auto'?input.locale:'en',items,quote,accountingQuote,id,subtotal,shippingDisplay,total,promotionCode:code,provider:'stripe',createdAt:Date.now()};
...
function preparedIntentState(pi,cart,id){
 if(pi.currency!==cart.quote.chargeCurrency.toLowerCase()||pi.amount!==cart.quote.chargeMinor||pi.metadata?.pmp_checkout_id!==id)throw error('Payment amount mismatch');
 const voucher=oxxoVoucher(pi);if(voucher)return {pending:true,voucher};
...
async function prepareIntentResponse(pi,cart,id){
 ...
 return {clientSecret:pi.client_secret,amount:pi.amount,currency:cart.quote.chargeCurrency};
}
...
export function buildStripeOrder(cart,payment,attempt){
 const order=buildOrder({...cart,quote:cart.accountingQuote||cart.quote},{id:payment.id},attempt);
 ...
 for(const attr of order.customAttributes){
  if(attr.key==='pmp_payment_currency')attr.value=cart.quote.chargeCurrency;
  if(attr.key==='pmp_payment_amount')attr.value=major(cart.quote.chargeMinor,cart.quote.chargeScale||100);
 }
...
export async function settleStripePayment(paymentId,eventCreated) {
  ...
  if(!payment.livemode || cart.provider!=='stripe' || payment.currency!==cart.quote.chargeCurrency.toLowerCase() || payment.amount!==cart.quote.chargeMinor || payment.amount_received!==cart.quote.chargeMinor || (attempt.paymentId && attempt.paymentId!==payment.id))throw error('Payment amount mismatch');
...
export async function pendingStripePayment(cart){
 const attempt=await get('stripe:attempt:'+cart.id);
 if(!attempt?.paymentId)return {};
 const pi=await stripe('/payment_intents/'+encodeURIComponent(attempt.paymentId));
 const state=preparedIntentState(pi,cart,cart.id);
 return state?.pending?{paymentPending:true,...(state.voucher?{voucher:state.voucher}:{})}:{};
}
