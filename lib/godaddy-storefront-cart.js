...
export async function recalculateShopifyCart(snapshot,{country,locale='en',root}={}){
 ...
 // Keep distinct bundle configurations, but combine paid/free fragments of the
 // same variant/configuration. Only the bounded merchandise properties survive.
 const quantities=new Map();
 for(const i of clean.items){const id=Number(i.variant_id),properties=i.properties||{},key=JSON.stringify([id,properties]);const existing=quantities.get(key)||{id,quantity:0,properties};existing.quantity+=Number(i.quantity);quantities.set(key,existing);}
 try{
  ...
  // A fresh anonymous cart has no previously applied promo. Reapply the code
  // through Shopify, which owns its validity and rounding. Never infer a price
  // or bypass equality checks because the browser claims a discount.
  if(codes.length){
