// Recalculate the merchandise cart in Shopify itself. No customer data, payment or order creation.
import {compactCart} from '../public/godaddy-cart-snapshot.js';
import {cartError} from '../public/godaddy-cart-contract.js';
export function storefrontRoot(country,locale='en'){
 return ({US:'',CA:'/en-ca',GB:'/en-gb',AU:'/en-au',FR:'/fr-fr',DE:'/de-de',ES:'/es-es',IT:'/it-it',PT:'/pt-pt',NZ:'/en-nz'})[country]??('/'+locale.toLowerCase().split('-')[0]+'-'+country.toLowerCase());
}
export async function recalculateShopifyCart(snapshot,{country,locale='en',root}={}){
 const path=root??storefrontRoot(country,locale);
 if(!/^\/(?:[a-z]{2}(?:-[a-z]{2})?\/?)?$/.test(path||'/'))throw cartError('INVALID_STOREFRONT_ROOT');
 ...
 try{
  ...
  const read=await request(base+'/cart.js');if(!read.ok||!read.headers.get('content-type')?.includes('application/json'))throw cartError('SHOPIFY_CART_RECALCULATION_FAILED');
  const actual=compactCart(await read.json());
  if(actual.currency!==snapshot.currency||actual.total_price!==snapshot.total_price)throw cartError('SHOPIFY_CART_TOTAL_CHANGED');
  ...
  if(JSON.stringify(groups(actual.items))!==JSON.stringify(groups(snapshot.items)))throw cartError('SHOPIFY_CART_LINES_CHANGED');
  return actual;
 }finally{
  // This jar belongs only to this server-side anonymous verification cart.
  try{await request(base+'/cart/clear.js',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});}catch{}
 }
}
