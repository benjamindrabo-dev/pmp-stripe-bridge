...
// checkout_country?, locale?, note?, email?, shopify_cart_token?, shopify_cart_url? }
// `currency` normally comes from the Shopify cart. A manual-removal US rule can
...
function containsEmail(value) {
  ...
  let candidate = value;
  // Attribution URLs can contain an encoded email address. Decode twice to
  // cover the common `%2540` form without ever throwing on malformed input.
  ...
  return /[A-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[A-Z0-9-]+(?:\.[A-Z0-9-]+)+/i.test(candidate);
}

// Campaign names and sources are browser-controlled. Keep normal spaces and
// punctuation used by ad platforms, but never copy an email address into
// Stripe metadata or Shopify attribution notes downstream.
function safeAttributionText(value, max = 500) {
  const clean = safeText(value, max);
  return clean && !containsEmail(clean) ? clean : null;
}

function safeOpaqueId(value, max = 255) {
  const clean = safeAttributionText(value, max);
  if (!clean || !/^[A-Za-z0-9._~-]+$/.test(clean)) return null;
  return clean;
}

// Google, Meta, Microsoft and TikTok click identifiers are opaque URL-safe
// values. A minimum length also rejects common injected placeholders such as
// `null`, `test` and single-character values.
function safeClickId(value, max = 255) {
  const raw = typeof value === "string" ? value.trim() : "";
  if (!raw || raw.length > max) return null;
  const clean = safeOpaqueId(raw, max);
  return clean && clean.length >= 6 ? clean : null;
}

function safeJourneyId(value) {
  const raw = typeof value === "string" ? value.trim() : "";
  if (!raw || raw.length > 255) return null;
  return safeOpaqueId(raw, 255);
}

// The campaign-aware wrapper can add two metadata fields after this handler
// builds the Stripe request. Stripe accepts at most 50 metadata keys, so the
// base handler deliberately reserves those two slots. The order below keeps
// correlation and paid-click evidence before lower-priority reporting detail.
const MAX_BASE_SESSION_METADATA_KEYS = 48;
const SESSION_METADATA_PRIORITY = Object.freeze([
  "tracking_version", "journey_id", "shopify_cart_token", "shopify_cart_url",
  "checkout_country", "checkout_locale", "display_currency", "charge_currency",
  "currency_override", "currency_override_until", "email_sha256", "person_id", "browser_id",
  "gclid", "gbraid", "wbraid", "dclid", "fbclid", "ttclid", "msclkid", "sccid",
  "fbp", "fbc", "ga_client_id", "ga_session_id", "ga_session_number", "attribution_model",
  "last_touch_landing", "last_touch_referrer", "last_touch_source", "last_touch_medium",
  "last_touch_campaign", "last_touch_at", "landing_page", "referrer", "utm_source",
  "utm_medium", "utm_campaign", "utm_content", "utm_term", "first_entry_landing",
  "first_entry_referrer", "first_entry_source", "first_entry_medium", "first_entry_campaign",
  "first_entry_at", "first_touch_landing", "first_touch_referrer", "first_touch_source",
  "first_touch_medium", "first_touch_campaign", "first_touch_at",
]);

// Both `_fbp` and `_fbc` use Meta's `fb.1.<timestamp>.<opaque-id>` format.
// Validating the complete value prevents arbitrary browser input (including
// PII) from being forwarded to Stripe and the Conversions API.
function safeFacebookBrowserId(value) {
  const clean = safeAttributionText(value, 255);
  if (!clean) return null;
  return /^fb\.1\.\d{10,16}\.[A-Za-z0-9._~-]{6,200}$/.test(clean) ? clean : null;
}

// Stripe accepts a finite locale enum rather than arbitrary IETF tags. Map the
// storefront's regional Shopify locales to that enum and fall back safely to
// English for missing or manipulated browser input.
const STRIPE_CHECKOUT_LOCALES = Object.freeze({
  en: "en",
  "en-ca": "en",
  "en-us": "en",
  "en-gb": "en-GB",
...
function safeCheckoutLocale(value) {
  const clean = safeText(value, 20);
  if (!clean) return "en";
  const normalized = clean.replace(/_/g, "-").toLowerCase();
  return STRIPE_CHECKOUT_LOCALES[normalized] || "en";
}

// Only borrow the market prefix from the cart, never its host or query. Keep
// the configured return destination and all non-Spanish checkouts unchanged.
export function spanishMarketReturnBase(configured, cartUrl, country, locale) {
  try {
    const target = new URL(configured);
    const cart = cartUrl ? new URL(cartUrl) : null;
    const hostname = (url) => url.hostname.toLowerCase().replace(/^www\./, "");
    const selectedSpain = country === "ES" && locale === "es";
    const spanishCart = cart && cart.protocol === "https:" &&
      hostname(target) === hostname(cart) && target.port === cart.port &&
      /^\/es-es\/cart(?:\/|$)/.test(cart.pathname);
    if (target.protocol !== "https:" || (!selectedSpain && !spanishCart)) return configured;
    if (!/^\/(?:[a-z]{2}(?:-[a-z]{2})?\/)?(?:pages\/)?thank-you\/?$/.test(target.pathname)) return configured;
    target.pathname = "/es-es/" + target.pathname.replace(/^\/(?:[a-z]{2}(?:-[a-z]{2})?\/)?/, "");
...
function safePageUrl(value) {
  ...
  try {
    ...
    u.hash = "";
    u.search = "";
    // Some email and CRM links put the customer's address in the path. The
    // origin remains useful for attribution; the identifying path does not.
...
async function contextualPricesFor(items, CUR, options = {}) {
  const cur = CUR.toUpperCase();
  if (!options.anyCurrency && !ALLOWED_CURRENCIES.includes(cur)) throw reject("Unsupported currency");
  const country = options.country || MARKET_COUNTRY[cur];
  if (!country || !/^[A-Z]{2}$/.test(country)) throw reject("Unsupported currency");
...
async function assertPricesLegit(items, CUR, knownPriceById = null, options = {}) {
  const cur = CUR.toUpperCase();
  if (!options.anyCurrency && !ALLOWED_CURRENCIES.includes(cur)) throw reject("Unsupported currency");
  ...
  const priceById = knownPriceById || await contextualPricesFor(items, cur, options);
  ...
  for (const it of items) {
    ...
    const unit = Number(it.price_cents) / (options.scale || 100);
    if (unit > 0) {
...
export default async function handler(req, res) {
  ...
  try {
    const {
      items: requestedItems, note, currency, checkout_country, locale, email,
      shopify_cart_token, shopify_cart_url, fbp, fbc, external_id,
      landing_url, referrer,
      utm_source, utm_medium, utm_campaign, utm_content, utm_term,
      journey_id, gclid, gbraid, wbraid, dclid, fbclid, ttclid, msclkid, sccid,
      ga_client_id, ga_session_id, ga_session_number,
      attribution_model,
      first_entry_landing_url, first_entry_referrer, first_entry_source,
      first_entry_medium, first_entry_campaign, first_entry_at,
    ...
    const checkoutLocale = safeCheckoutLocale(locale);
    ...
    const squareMode = req.squareMode === true;
    const scale = squareMode ? 10 ** Math.max(2, new Intl.NumberFormat("en", {style:"currency",currency:displayCurrency}).resolvedOptions().maximumFractionDigits) : 100;
    if (squareMode && requestedItems.some(it => !Number.isSafeInteger(Number(it.price_cents)))) throw reject("Invalid price");
    const displayValidation = await assertPricesLegit(requestedItems, displayCurrency, null, squareMode ? {anyCurrency:true,country:checkoutCountry,scale} : {});
    let items = displayValidation.items;
    let CUR = displayCurrency.toLowerCase();
    let currencyOverride = null;

    if (!squareMode && shouldApplyUsCadOverride({ displayCurrency, checkoutCountry })) {
    ...
    const returnBase = spanishMarketReturnBase(
      process.env.SUCCESS_URL || "https://example.com/thank-you", shopifyCartUrl,
      checkoutCountry, checkoutLocale
    );
    params.append(
      "return_url",
    ...
    const attribution = {
      ...
      marketing_allowed: req.body.marketing_allowed === true,
      ...
      person_id: safeOpaqueId(external_id, 64),
      browser_id: safeOpaqueId(external_id, 64),
      fbp: safeFacebookBrowserId(fbp),
      fbc: safeFacebookBrowserId(fbc),
      landing_page: safePageUrl(landing_url),
      referrer: safePageUrl(referrer),
      utm_source: safeAttributionText(utm_source, 100),
      utm_medium: safeAttributionText(utm_medium, 100),
      utm_campaign: safeAttributionText(utm_campaign, 200),
      utm_content: safeAttributionText(utm_content, 200),
      utm_term: safeAttributionText(utm_term, 200),
      journey_id: safeJourneyId(journey_id),
      gclid: safeClickId(gclid),
      gbraid: safeClickId(gbraid),
      wbraid: safeClickId(wbraid),
      dclid: safeClickId(dclid),
      fbclid: safeClickId(fbclid),
      ttclid: safeClickId(ttclid),
      msclkid: safeClickId(msclkid),
      sccid: safeClickId(sccid),
      ...
      ga_client_id: safeOpaqueId(ga_client_id, 64),
      ga_session_id: safeOpaqueId(ga_session_id, 64),
      ga_session_number: safeOpaqueId(ga_session_number, 64),
      attribution_model: safeOpaqueId(attribution_model, 100),
      first_entry_landing: safePageUrl(first_entry_landing_url),
      first_entry_referrer: safePageUrl(first_entry_referrer),
      first_entry_source: safeAttributionText(first_entry_source, 100),
      first_entry_medium: safeAttributionText(first_entry_medium, 100),
      first_entry_campaign: safeAttributionText(first_entry_campaign, 200),
      first_entry_at: safeAttributionText(first_entry_at, 40),
      first_touch_landing: safePageUrl(first_touch_landing_url),
      first_touch_referrer: safePageUrl(first_touch_referrer),
      first_touch_source: safeAttributionText(first_touch_source, 100),
      first_touch_medium: safeAttributionText(first_touch_medium, 100),
      first_touch_campaign: safeAttributionText(first_touch_campaign, 200),
      first_touch_at: safeAttributionText(first_touch_at, 40),
      last_touch_landing: safePageUrl(last_touch_landing_url),
      last_touch_referrer: safePageUrl(last_touch_referrer),
      last_touch_source: safeAttributionText(last_touch_source, 100),
      last_touch_medium: safeAttributionText(last_touch_medium, 100),
      last_touch_campaign: safeAttributionText(last_touch_campaign, 200),
      last_touch_at: safeAttributionText(last_touch_at, 40),
    };
    if (squareMode) {
      const createSquareQuote = req.stripeCadMode ? (await import("./stripe-cad-bridge.js")).createStripeQuote : (await import("./square-bridge.js")).createSquareQuote;
      return res.status(200).json(await createSquareQuote({
        items, displayCurrency, scale, country: checkoutCountry, locale: checkoutLocale,
        catalogPrices: displayValidation.priceById, suggestions: Array.isArray(req.body.square_suggestions)?req.body.square_suggestions.slice(0,6):[],
        attribution, email: shopperEmail, note: safeText(note, 1000),
        ua: req.headers["user-agent"] || null,
        ip: String(req.headers["x-forwarded-for"] || "").split(",")[0].trim() || null,
        promotionCode: req.body.promotion_code || null,
      }));
    }
    let sessionMetadataCount = 0;
    for (const key of SESSION_METADATA_PRIORITY) {
      const value = attribution[key];
      if (!value) continue;
      params.append(`metadata[${key}]`, value);
      sessionMetadataCount += 1;
      if (sessionMetadataCount >= MAX_BASE_SESSION_METADATA_KEYS) break;
    }
    ...
    await kvSet(`sess:${data.id}`, {
      ...
      display_items: displayValidation.items.map(it => ({ variant_id: it.variant_id, quantity: it.quantity, price_cents: Number(it.price_cents) })),
      checkout_country: checkoutCountry,
      checkout_locale: checkoutLocale,
      currency_override: currencyOverride && currencyOverride.code,
      note: note || "",
      ...
      first_entry_landing: attribution.first_entry_landing,
      first_entry_referrer: attribution.first_entry_referrer,
      first_entry_source: attribution.first_entry_source,
      first_entry_medium: attribution.first_entry_medium,
      first_entry_campaign: attribution.first_entry_campaign,
      first_entry_at: attribution.first_entry_at,
    ...
    const beginCheckoutEventId = deterministicEventId("begin checkout", data.id);
    const checkoutValueCents = items.reduce(
      (total, item) => total + (Number(item.price_cents) * Number(item.quantity || 1)),
      0,
    );
    const beginCheckoutItems = items.map((item) => ({
      item_id: String(Number(item.variant_id)),
      item_name: String(item.title || "Item").slice(0, 250),
      price: Number((Number(item.price_cents) / 100).toFixed(2)),
    ...
    return res.status(200).json({
      ...
      analytics: {
        beginCheckout: {
          eventId: beginCheckoutEventId,
          currency: String(data.currency || CUR).toUpperCase(),
          value: Number((checkoutValueCents / 100).toFixed(2)),
