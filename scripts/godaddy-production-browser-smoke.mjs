...
// No card data, payment submission, order creation, cookies or secrets in output.
import {chromium} from 'playwright';
import assert from 'node:assert/strict';
import fs from 'node:fs';
const SITE='https://www.puremajestypet.com',CHECKOUT='https://checkout.puremajestypet.com';
...
try{
 ...
 for(const scenario of scenarios){
  ...
  await context.route('**/*',async route=>{
   ...
   if(req.method()==='POST'&&(u.pathname==='/api/godaddy-checkout'||/\/cards\/tokenize\/charge|\/payments$|\/payment_intents(?:\/|$)/.test(u.pathname))){result.blockedMonetaryRequest=true;return route.abort('blockedbyclient');}
   return route.continue();
  });
  const page=await context.newPage();let bridgeResponse=null;
  page.on('response',async response=>{try{const u=new URL(response.url());if(u.pathname==='/api/create-checkout'&&response.request().method()==='POST'){const d=await response.json();bridgeResponse={status:response.status(),provider:d.paymentProvider||null,sessionId:d.sessionId||null,amountTotal:d.amountTotal,currency:d.currency,code:d.code||null};}}catch{}});
  try{
   ...
   const add=await context.request.post(SITE+scenario.root+'/cart/add.js',{data:{items:scenario.items}});result.cartAddStatus=add.status();assert.ok(add.ok(),'CART_ADD_FAILED');
   const raw=await(await context.request.get(SITE+scenario.root+'/cart.js')).json();result.cartCurrency=raw.currency;result.cartTotalMinor=raw.total_price;result.cartQuantity=raw.item_count;
   await page.goto(SITE+scenario.root+'/cart',{waitUntil:'domcontentloaded',timeout:60000});await page.waitForTimeout(2000);
   for(const label of [/reject all/i,/decline/i,/necessary only/i]){try{const b=page.getByRole('button',{name:label}).first();if(await b.isVisible())await b.click({timeout:1500});}catch{}}
   result.stage='native-checkout-button';
   // The actual production cart labels its checkout action "Buy Now".
   const candidates=[page.locator('button[name="checkout"]'),page.locator('input[name="checkout"]'),page.locator('#checkout'),page.getByRole('button',{name:/^(buy now|check out|checkout|proceed to checkout|secure checkout|passer à la caisse|paiement)$/i}),page.locator('a[href$="/checkout"]')];
   let button;
   for(const candidate of candidates){for(let i=0;i<await candidate.count();i++){const b=candidate.nth(i);if(await b.isVisible()&&await b.isEnabled()){button=b;break;}}if(button)break;}
   if(!button){result.visibleButtons=(await page.locator('button:visible').allTextContents()).map(t=>t.trim().slice(0,70)).slice(0,25);throw Error('CHECKOUT_BUTTON_NOT_FOUND');}
   result.clickedLabel=(await button.textContent()||await button.getAttribute('value')||'').trim().slice(0,80);
   await button.click({timeout:15000});result.stage='checkout-navigation';
   ...
   result.bridge=bridgeResponse;const u=new URL(page.url());result.checkoutPath=u.pathname;result.sessionId=u.searchParams.get('session_id');
   assert.equal(bridgeResponse?.status,200,'BRIDGE_FAILED');assert.equal(bridgeResponse.provider,'godaddy','WRONG_PROCESSOR');result.stage='render-live-quote';
   await page.waitForFunction(()=>document.querySelectorAll('#items .product').length>0,null,{timeout:30000});
   ...
   assert.equal(await page.locator('#items .product').count(),q.items.length);
   await page.waitForFunction(()=>document.querySelector('#card-element')?.dataset.sdkReady==='true',null,{timeout:40000});
   ...
   if(scenario.country==='CA'){
    ...
    await page.waitForFunction(()=>document.querySelectorAll('.remove-addon').length===1,null,{timeout:60000});result.dentalAdded=true;
