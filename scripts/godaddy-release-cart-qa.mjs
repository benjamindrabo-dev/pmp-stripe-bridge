// Preview-only cart acceptance. No card or order submission; no secrets logged.
...
async function run(){
 ...
 if(process.env.VERCEL_ENV!=='preview'||!['release/godaddy-live-20260910','prep/godaddy-payments-20260909'].includes(process.env.VERCEL_GIT_COMMIT_REF))throw Error('PREVIEW_BRANCH_REQUIRED');
 ...
 async function session(country,path,item){
  ...
  async function request(route,init={}){
   ...
   if(!response.ok&&response.status!==302)item.publicHttpFailure={path:route.split('?')[0],status:response.status};
 ...
 for(const scenario of scenarios){
  const item={name:scenario.name,stage:'localize'};report.cases.push(item);let request;
  try{
   request=await session(scenario.country,scenario.path,item);item.stage='cart_add';
   const added=await request(scenario.path+'/cart/add.js',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({items:scenario.items})});item.cartAddHttp=added.status;if(!added.ok)throw Error('CART_ADD_HTTP_'+added.status);
   item.stage='cart_read';const response=await request(scenario.path+'/cart.js');item.cartReadHttp=response.status;if(!response.ok)throw Error('CART_READ_HTTP_'+response.status);const raw=await response.json(),cart=compactCart(raw);
   item.stage='entry';const created=await endpoint(entry,{pmp_cart:cart,currency:cart.currency,checkout_country:scenario.country,locale:scenario.locale,storefront_root:scenario.path||'/',marketing_allowed:false});
   ...
   item.stage='quote_assertions';const loaded=await endpoint(cartHandler,null,{session_id:created.data.sessionId});assert.equal(loaded.code,200);const q=loaded.data;
   ...
   const saved=await loadCart(q.sessionId);item.stage='order_payload_only';
   ...
   Object.assign(item,{stage:'complete',passed:true,currency:q.currency,cartTotal:cart.total_price,checkoutTotal:q.total,difference:q.subtotal-cart.total_price,chargeMinor:q.chargeMinor,sessionId:q.sessionId,quantity:cart.item_count,orderPayloadOnly:true});if(!first)first=q;
  }catch(e){item.passed=false;item.error=/^[A-Z0-9_]+$/.test(e.message||'')?e.message:'ASSERTION_OR_NETWORK_FAILED';}
 ...
 if(first){try{
  ...
  const stale=await endpoint(cartHandler,{action:'add-dental',sessionId:first.sessionId});assert.notEqual(stale.code,200);report.cases.push({name:'dental-add-remove-stale-guard',passed:true});
  const promo=await endpoint(cartHandler,{action:'promotion',sessionId:removed.data.sessionId,code:'WELCOME20'});assert.equal(promo.code,200);assert.equal(promo.data.total,Math.round(first.subtotal*.8)+first.shipping);report.cases.push({name:'promotion',passed:true,before:first.total,after:promo.data.total});
  const blocked=await endpoint(paymentHandler,{action:'prepare',sessionId:promo.data.sessionId});assert.equal(blocked.code,503);report.cases.push({name:'preview-payment-blocked',passed:true});
