...
function fixture(t,{lostResponse=false}={}){
 ...
 globalThis.fetch=async(url,options={})=>{
  ...
  if(u==='https://api.stripe.com/v1/payment_intents'&&options.method==='POST'){
   ...
   if(!pi){counts.created++;pi={id:'pi_mock',status:'requires_payment_method',currency:'cad',amount:4000,amount_received:0,metadata:{pmp_checkout_id:id,pmp_provider:'stripe_cad'},receipt_email:p.get('receipt_email'),livemode:true,client_secret:'pi_mock_secret_TEST_ONLY'};intents.set(idem,pi);createResponses.set(idem,structuredClone(pi));}
