...
test('Stripe surface preserves local formatter and uses the frozen payment currency and amount',()=>{const s=source('public/stripe-checkout.js');assert.ok(s.includes('currency:data.quote.displayCurrency'));assert.ok(s.includes('currency:data.quote.chargeCurrency.toLowerCase(),amount:data.quote.chargeMinor'));assert.ok(s.includes('stripeClient.confirmPayment'));assert.ok(!s.includes('Square.payments'));assert.ok(s.includes('elements.submit()'));});
